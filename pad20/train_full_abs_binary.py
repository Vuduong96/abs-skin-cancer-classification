# -*- coding: utf-8 -*-
"""
PAD-UFES-20 — ABS (Adaptive Balanced Sampling), patient-level split,
binary classification task (cancer vs. non-cancer).
...
BASE_OUTPUT   = "outputs/pad20_binary"
...
def run_ablation(config_name: str, cfg: dict,
    ...
    for round_idx in range(1, total_rounds + 1):
        ...
        mel_f1 = metrics.get("f1_cancer", 0.0)   # binary: track "cancer"-class F1 (minority-ish, clinically critical)
        ...
        logger.info(
            ...
            f"cancer={mel_f1:.4f} | "
            f"non_cancer={metrics.get('f1_non_cancer', 0):.4f} | "
    ...
    logger.info(f"\n  [{config_name}] DONE | "
                ...
                f"Best Cancer F1: {best_mel_f1:.4f}")
...
def main():
    ...
    # BINARY: collapse the 6-way diagnostic to PAD-UFES-20's own standard
    # cancer/non-cancer grouping (same mapping as
    # PAD20_EFFICIENTNETB3_PSO_PATIENT_SPLIT_BINARY.py, for apples-to-apples
    # comparison): BCC/SCC/MEL = cancer, ACK/NEV/SEK = non_cancer.
    logger.info(f"Raw 6-way diagnostic counts: {meta['diagnostic'].value_counts().to_dict()}")
    CANCER_CLASSES = {"BCC", "SCC", "MEL"}
    meta["diagnostic"] = meta["diagnostic"].apply(
        lambda d: "cancer" if d in CANCER_CLASSES else "non_cancer")
    logger.info(f"Binary diagnostic counts: {meta['diagnostic'].value_counts().to_dict()}")

    le             = LabelEncoder()
    meta["label"]  = le.fit_transform(meta["diagnostic"])
    meta["class_name"] = meta["diagnostic"].astype(str)
    class_names    = list(le.classes_)   # [cancer, non_cancer]
